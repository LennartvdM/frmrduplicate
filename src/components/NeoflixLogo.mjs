/**
 * NeoflixLogo — Standalone animated SVG logo component.
 *
 * Reproduces the original Framer "Neoflix anim" component (page--home.mjs:62–307)
 * using only React + framer-motion. Zero Framer runtime dependencies.
 *
 * Animation sequence:
 *   1. Mount: Inner Ring at rotate: -62°, Outer Ring at rotate: 97°
 *   2. After 400ms: both spring-animate to rotate: 0° (SPRING_HEAVY)
 *   3. Hover: Inner twitches to -6°, Outer to +5° (subtle counter-rotation)
 *   4. Click: rings snap back to initial rotations, then re-animate after 400ms
 *
 * Spring config (SPRING_HEAVY): damping: 24, mass: 9, stiffness: 500
 * This produces a slow, weighty spring — the signature Neoflix "unfurl" feel.
 */
...
// -- SVG paths (extracted from page--home.mjs:257, 273) --

const INNER_RING_SVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 395.143 395.143">
  <g>
...
/**
 * NeoflixLogo component.
 *
 * @param {Object} props
 * @param {string} [props.color="#48c1c4"] - Brand teal color for the SVG fills
 * @param {string} [props.wordmarkColor="#1c3664"] - Dark blue for the "Neoflix" wordmark
 * @param {number} [props.width=935] - Container width (matches original default)
 * @param {number} [props.height=287] - Container height (matches original default)
 * @param {number} [props.autoPlayDelay=400] - ms before the "unfurl" animation starts
...
export default function NeoflixLogo({
  ...
  // "Variant 1" = rings at initial rotation, "Variant 2" = rings at 0°
  const [variant, setVariant] = useState("initial");

  // Auto-transition to "open" after mount delay
  useEffect(() => {
    const timer = setTimeout(() => setVariant("open"), autoPlayDelay);
    return () => clearTimeout(timer);
  }, [autoPlayDelay]);

  // Click: snap back to initial, then re-animate
  const handleTap = useCallback(() => {
    onClick?.();
    setVariant("initial");
    setTimeout(() => setVariant("open"), autoPlayDelay);
  }, [onClick, autoPlayDelay]);

  // Ring rotation values per variant
  const innerRotation = variant === "initial" ? -62 : 0;
  const outerRotation = variant === "initial" ? 97 : 0;

  // Hover offsets (only when open)
  const innerHover = variant === "open" ? -6 : 0;
  const outerHover = variant === "open" ? 5 : 0;
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        height,
        overflow: "visible",
        cursor: variant === "open" ? "pointer" : "default",
        ...style,
      }}
      onTap={handleTap}
    ...
    >
      ...
      <motion.div
        ...
        whileHover={{ rotate: innerRotation + innerHover }}
        transition={SPRING_HEAVY}
        dangerouslySetInnerHTML={{ __html: INNER_RING_SVG }}
      />
      ...
      <motion.div
        ...
        whileHover={{ rotate: outerRotation + outerHover }}
        transition={SPRING_HEAVY}
        dangerouslySetInnerHTML={{ __html: OUTER_RING_SVG }}
      />
