 ...
 *
 * Reverse-engineered from the Framer "Neoflix anim" component (page--home.mjs:62–307).
 * Uses only React + framer-motion. Zero Framer runtime dependencies.
 *
 ...
 *
 * Animation sequence:
 *   1. Mount: rings start splayed apart (Inner: -62°, Outer: +97°)
 *   2. After 400ms: both spring to 0° — the "unfurl" / assembly
 *   3. Hover (anywhere on logo): rings counter-rotate slightly (Inner: -6°, Outer: +5°)
 *   4. Click: rings snap back to splayed positions, then reassemble after 400ms
 *
...
// ---------------------------------------------------------------------------
// SVG markup (extracted from page--home.mjs:257, 273, 288)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Full combined logo SVG (used in navbar — page--home.mjs:4090)
// ---------------------------------------------------------------------------
...
/**
 * @param {Object} props
 * @param {string}   [props.color="#48c1c4"]       — Teal for the ring SVGs
 * @param {string}   [props.wordmarkColor="#1c3664"] — Dark blue for the wordmark
 * @param {number}   [props.autoPlayDelay=400]     — ms before the unfurl animation
 * @param {Function} [props.onClick]               — Called on click (before the "explode" animation)
...
export default function NeoflixLogo({
  ...
  const innerTarget = !assembled
    ? INNER_RING.splayed                         // -62° (exploded)
    : hovered
      ? INNER_RING.hover                         // -6°  (gentle nudge)
      : INNER_RING.assembled;                    //  0°  (resting)

  const outerTarget = !assembled
    ? OUTER_RING.splayed                         // +97° (exploded)
    : hovered
