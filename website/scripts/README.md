...
| `build-route-html.mjs` | Gives every route its own HTML file with the right title, description and link-preview tags, then writes the sitemap, `robots.txt`, `404.html` and the redirects for old URLs. Driven by `shared/routeMeta.js`. |
